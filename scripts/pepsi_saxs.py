...
# PEPSI command run: pepsi-saxs frame.pdb SAXS_bift.dat -o saxs_frame.dat -cst -cstFactor 0 --I0 1.0 --dro 1.0 --r0_min_factor 1.025 --r0_max_factor --r0_N 1
...
def run_pepsi(
    ...
    if saxs.endswith(".csv"):
        ...
        expt_df = np.loadtxt(saxs)
        expt_df = pd.DataFrame(expt_df,columns=['q', 'I(q)', 'sigma'])
    else:
    ...
    for i, frame in enumerate(tqdm(trj)):
        ...
        #cmd = f"{pepsi} {frame_file_i} {saxs_tmp} -o {dat_file_i} -cst --cstFactor 0 --I0 1.0 --dro 1.0 --r0_min_factor 1.025 --r0_max_factor 1.025 --r0_N 1"
        cmd = [pepsi, frame_file_i, saxs_tmp, "-o", dat_file_i, "-cst", "--cstFactor", "0", "--I0", "1.0", "--dro", "1.0", "--r0_min_factor", "1.025", "--r0_max_factor", "1.025", "--r0_N", "1"]
        try:
